      ...
      jQuery(tag).find('img, input[type=image]').not('.'+ Olwimpics._class_name).each(function() {
        try {
          ...
          if ((r.attr('src') +' '+ r.attr('alt') +' '+ r.attr('title')).match(Olwimpics._regexp)) {
            ...
            r.addClass(Olwimpics._class_name).css({'background' : c}).attr('src', 'http://assets.gleuch.com/blank.png').attr('title', Olwimpics._blocked_text).attr('alt', Olwimpics._blocked_text).width(w).height(h);
          }
        } catch(e) {}
      });
    });
