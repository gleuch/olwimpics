    ...
    jQuery(tag).find('img, input[type=image]').each(function() {
      try {
        ...
        if (!r.hasClass(Olwimpics._class_name) && text.match(Olwimpics._regexp)) {
          ...
          r.addClass(Olwimpics._class_name).css({'background': Olwimpics.medal() +' !important', 'width': w+'px !important', 'height': h+'px !important'}).attr('src', 'http://assets.gleuch.com/blank.png').attr('title', Olwimpics._blocked_text).attr('alt', Olwimpics._blocked_text).width(w).height(h);
        }
      } catch(e) {}
    });
  }
